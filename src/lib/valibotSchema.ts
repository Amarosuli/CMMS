...
export const MaterialMasterSchema = object({
	code: pipe(string(), nonEmpty('Please enter the code'), trim()),
	description: pipe(string(), nonEmpty('Please enter the description'), trim()),
...
export const StockInSchema = pipe(
	object({
	...
	forward(
		check((input) => (input.isPackaged ? !!input.package_size : true), 'Package size  required when item is packaged'),
		['package_size']
	),
	forward(
		check((input) => (input.isPackaged ? !!input.package_name : true), 'Package name required when item is packaged'),
		['package_name']
	)
);
