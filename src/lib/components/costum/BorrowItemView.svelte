...
<div class="flex items-center gap-3 border-b py-2">
	<div class="flex max-w-80 min-w-80 flex-col justify-center">
		<p class="text-xs font-semibold">{item.expand?.stock_item_id.expand.stock_master_id.expand.material_master_id.code} - <span class="text-primary">{item.expand?.stock_item_id.expand.stock_master_id.expand.material_master_id.part_number}</span></p>
		<p class="text-xs">{item.expand?.stock_item_id.expand.stock_master_id.expand.material_master_id.description}</p>
	</div>
	<p class="flex h-full min-w-20 flex-row gap-2 self-center text-sm">Qty : {item.quantity_out} {item.expand?.stock_item_id.expand.stock_master_id.expand.material_master_id.expand.material_unit_id.code ?? ''}</p>
	<p class="flex h-full min-w-20 flex-row gap-2 self-center text-sm">Qty : {item.quantity_return} {item.expand?.stock_item_id.expand.stock_master_id.expand.material_master_id.expand.material_unit_id.code ?? ''}</p>
	<div class="ml-auto flex min-w-40 flex-col items-center">
		<p class="flex h-full flex-row gap-2 self-start text-xs">BN <span class="text-primary">{item.expand?.stock_item_id.expand.stock_master_id.batch_number}</span></p>
		<p class="flex h-full flex-row gap-2 self-start text-xs">PO <span>{item.expand?.stock_item_id.expand.stock_master_id.purchase_order}</span></p>
	</div>
