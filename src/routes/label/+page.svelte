...
<div class="mt-4 flex w-full gap-2">
	...
	<Button
		variant="outline"
		disabled={labelCarts.data.length ? false : true}
		onclick={() => {
...
{#if !isPDFPreviewOpen}
	...
	<div class="mt-4 hidden w-full flex-wrap lg:flex">
		{#each labelCarts.data as label (label.label)}
			<Tooltip.Root disableCloseOnTriggerClick>
				<Tooltip.Trigger class="relative flex w-1/5 shrink-0 flex-col items-center justify-center border-[0.5px] py-2 text-[0.75rem]">
					<Button class="absolute right-2 bottom-2 bg-red-200/50 hover:bg-red-200 dark:bg-red-400/50  dark:hover:bg-red-200" size="sm" variant="ghost" onclick={() => removeFromLabelCart(label.label)}><X class="size-4 font-semibold text-red-500" /></Button>
					<div class="flex items-center justify-center">
						<canvas use:renderBarcode={label.label}></canvas>
					</div>
