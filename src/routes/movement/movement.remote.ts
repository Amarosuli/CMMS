...
export const getRecentMovements = query(async () => {
	...
	const { status: stockInStatus, data: stockIn } = await tryCatch(locals.pb.collection('stock_in').getFullList({ expand: 'user_id', filter: `created>"${new Date().toISOString().split('T')[0]}"` }));
	const { status: stockOutStatus, data: stockOut } = await tryCatch(locals.pb.collection('stock_out').getFullList({ expand: 'stock_id,stock_id.stock_in_id,user_id', filter: `created>"${new Date().toISOString().split('T')[0]}"` }));
