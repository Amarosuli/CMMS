<script lang="ts">
	...
	const { stockMaster } = $derived(data);

	let material = stockMaster?.expand?.material_id ? { ...stockMaster.expand.material_id } : { code: '-', part_number: '-', description: '-' };
	let unit = stockMaster?.expand?.material_id?.expand?.unit_id ? { code: stockMaster.expand.material_id.expand.unit_id.code } : { code: '' };
	let qtyAvailableInStore = stockMaster?.quantity_borrowed && stockMaster?.quantity_available ? `: ${stockMaster.quantity_available - stockMaster.quantity_borrowed} ${unit.code}` : '';
...
<div class="mt-12">
	...
	<dl class="grid grid-cols-1 text-base/6 sm:grid-cols-[min(50%,--spacing(80))_auto] sm:text-sm/6">
		...
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{data.stockMaster?.batch_number}</dd>
		<dt class="col-start-1 border-t border-foreground/5 pt-3 text-foreground/50 first:border-none sm:py-3">Purchase Order</dt>
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{data.stockMaster?.purchase_order}</dd>
		...
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{data.stockMaster?.quantity_available} {unit?.code} ( Available in store{qtyAvailableInStore} )</dd>
		<dt class="col-start-1 border-t border-foreground/5 pt-3 text-foreground/50 first:border-none sm:py-3">Borrowed Quantity</dt>
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{data.stockMaster?.quantity_borrowed} {unit.code}</dd>
		<dt class="col-start-1 border-t border-foreground/5 pt-3 text-foreground/50 first:border-none sm:py-3">Expired Date</dt>
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{data.stockMaster?.expired_date}</dd>
		<dt class="col-start-1 border-t border-foreground/5 pt-3 text-foreground/50 first:border-none sm:py-3">Material Code</dt>
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{material.code}</dd>
		...
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{material.part_number}</dd>
		...
		<dd class="pt-1 pb-3 text-foreground sm:border-t sm:border-foreground/5 sm:py-3 sm:nth-2:border-none">{material.description}</dd>
	</dl>
</div>
