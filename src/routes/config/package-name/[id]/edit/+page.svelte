...
<svelte:head>
	<title>CMMS - Edit Package Name</title>
</svelte:head>

<div>
	<Button href="/config/package-name" variant="outline" class="inline-flex items-center gap-2 text-sm/6">
...
<div class="mt-4 lg:mt-8">
	<div class="flex items-center gap-4">
		<h1 class="text-2xl/8 font-semibold sm:text-xl/8">Edit <span class="text-foreground/50">Package Name</span></h1>
		...
		<Field {form} name="name">
			<Control>
				{#snippet children({ props })}
					<Label>Name</Label>
					<Input {...props} bind:value={$formData.name} type="text" placeholder="Package Name" />
				{/snippet}
			</Control>
		...
		<Field {form} name="description">
			<Control>
				{#snippet children({ props })}
					...
					<Input {...props} bind:value={$formData.description} type="text" placeholder="Package Description" />
				{/snippet}
			</Control>
			<FieldErrors class="text-xs italic" />
		</Field>
