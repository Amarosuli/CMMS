...
{#if materialUnit}
	<div class="mt-12">
		<h2 class="text-foreground text-base/7 font-semibold sm:text-sm/6">Detail of Material Unit</h2>
		...
		<dl class="grid grid-cols-1 text-base/6 sm:grid-cols-[min(50%,theme(spacing.80))_auto] sm:text-sm/6">
			...
			<dd class="text-foreground sm:border-foreground/5 pt-1 pb-3 sm:border-t sm:py-3 sm:[&:nth-child(2)]:border-none">{materialUnit.code.toUpperCase()}</dd>
			<dt class="border-foreground/5 text-foreground/50 col-start-1 border-t pt-3 first:border-none sm:py-3">Description</dt>
			<dd class="text-foreground sm:border-foreground/5 pt-1 pb-3 capitalize sm:border-t sm:py-3 sm:[&:nth-child(2)]:border-none">{materialUnit.description?.toLowerCase() || '-'}</dd>
		</dl>
	</div>
{/if}
